...
setwd("/Users/Jojo/Dropbox/Jojo/Postdoc/Metagenomics/Global_metagenomes_datasets/Binning/MyCC/Best_reciprocal_BLAST/Chol1")
...
while (fileCounter <= length(fileList)){
  ...
  SubjectAccession = sub(".csv.out$", "", fileList[fileCounter]) # Remove .csv suffix from csv filename. This should be the subject organism accession.
...
#HeatmapMatrix[HeatmapMatrix <= 30] <- NA
...
# HeatmapMatrix2 = HeatmapMatrix[order(rownames(HeatmapMatrix)), sort] # Reorders rows by gene name.
HeatmapMatrix2 = HeatmapMatrix[order(rownames(HeatmapMatrix)),]
